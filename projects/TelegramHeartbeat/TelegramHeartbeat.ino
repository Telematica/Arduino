...
unsigned int pokemonCounter = 0;
size_t pokemonCount = sizeof(pokemons) / sizeof(pokemons[0]);
...
// WiFi connection status check and reconnection attempt
void drawScreen_0()
{
  ...
  display.drawBitmap(111, 4, connected ? image_checked_bits : image_crossed_bits, 14, 16, 1);

  // Online status
  display.drawBitmap(111, 25, Ping.hasSuccess() ? image_checked_bits : image_crossed_bits, 14, 16, 1);

  // music_radio_streaming
  display.drawBitmap(72, 4, image_music_radio_streaming_bits, 17, 16, 1);

  // earth
  display.drawBitmap(73, 25, image_earth_bits, 15, 16, 1);
  ...
  display.print(Ping.hasSuccess() ? String(Ping.averageTime() > 0 ? Ping.averageTime() / 1000.0 : 0) + " s" : "N/A");
...
void showPokemonScreen(void)
{
  if (pokemonCounter >= pokemonCount)
  {
    pokemonCounter = 0;
  }
  display.clearDisplay();
  // paint 4
  display.drawBitmap(32, 0, pokemons[pokemonCounter], 64, 64, SSD1306_WHITE);
  // string 5
  display.setTextSize(1);
  display.setTextColor(1);
  display.setTextWrap(false);
  ...
  display.print("p");
  // string 7 copy 3
  display.setCursor(118, 28);
  display.print("d");
  // string 8 copy 3
  display.setCursor(118, 38);
  display.print("a");
  // string 6 copy 4
  display.setCursor(118, 47);
  display.print("t");
  // string 7 copy 4
...
void attachDetachInterrupt()
{
  if (eventTriggered)
  {
    previousMillis4 = currentMillis; // Reset the screen change timer to avoid immediate screen change after button press
